...
#### EESMÄRK ####
# 
# Kammime internetist e-poodide toodete hinnad, toodete otsingu teeme poodide lehtedel
...
for (i in 1:length(andmed_välja)) {
  ...
  # Jagame andmed data frame-i
  
  if (length(toode) == 3) {  # Kui on kolm eri osa toote stringis (kilohinda pole)
  ...
  if (length(toode) == 4) { # Kui on neli eri osa toote stringis (tavaline)
  ...
  if (length(toode) == 5) {
    ...
    df_prisma$Tootenimi[i] <- toode[1]
    df_prisma$Hind[i] <- toode[2]
    df_prisma$Hind_kg_kohta[i] <- toode[4]
...
while (TRUE) {
  ...
  # võtame andmed sellelt lehelt
  andmed <- rem_driver$findElements(using = "css selector", value = "li[class='tw-flex-shrink-0 tw-list-none tw-w-full']")
...
while (TRUE){
  ...
  tryCatch({
      ...
      # defineerime elemendi mida mööda alla kerime
      scrollable <- rem_driver$findElement(using = "xpath", value = "//html[@class='cb_semanticCssVars_sl0l']")
...
# Eemaldame read, kus Hind on NA. See juhtub Barbora puhul, kui neil on kirjas, et "Hetkel toodet kahjuks ei ole."
# Lisaks ka juhul, kui mõnes poes ei ole ole ühtegi toodet sellele otsingusõnale

bool <- is.na(df_tulemus$Hind)
